...
import { Image, Plus, Trash2, Star, Save } from 'lucide-react';
...
export default function InmuebleFormulario({ id, onCreado, onActualizado }) {
  ...
  const guardar = async (datos) => {
    ...
    try {
      if (editando) {
        ...
        toast.exito('Inmueble actualizado correctamente.');
        onActualizado?.();
      } else {
        const creado = await inmueblesServicio.crear(payload);
        toast.exito('Inmueble creado correctamente. Ahora puedes agregar imágenes.');
        onCreado?.(creado.inmueble._id);
      }
    } catch (error) {
      toast.error(error.response?.data?.mensaje || 'No se pudo guardar el inmueble');
    }
  };
  ...
  const subirImagen = async () => {
    ...
    try {
      ...
      await cargarDatos();
      toast.exito('Imagen agregada correctamente.');
    } catch (error) {
  ...
  const eliminarImagen = async (imagenId) => {
    ...
    try {
      ...
      await cargarDatos();
      toast.exito('Imagen eliminada correctamente.');
    } catch (error) {
      toast.error(error.response?.data?.mensaje || 'No se pudo eliminar la imagen');
    }
  };
  ...
  const establecerPrincipal = async (imagenId) => {
    try {
      ...
      toast.error(error.response?.data?.mensaje || 'No se pudo establecer la imagen principal');
    }
  };
